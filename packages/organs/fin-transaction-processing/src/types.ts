/**
 * TransactionProcessing Organ — Domain Types
...
// Domain Command
export interface TransactionProcessingCommand {
  id: string;
  type: string;
...
export interface TransactionProcessingEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-FIN-TRANSACTION_PROCESSING';
  version: number;
  causationId: string;
}

// Domain Query
export interface TransactionProcessingQuery {
  type: string;
...
export interface TransactionProcessingAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface TransactionProcessingHealth {
  organId: 'ORGX-FIN-TRANSACTION_PROCESSING';
