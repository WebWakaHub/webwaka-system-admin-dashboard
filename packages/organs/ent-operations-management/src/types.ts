/**
 * OperationsManagement Organ — Domain Types
...
// Domain Command
export interface OperationsManagementCommand {
  id: string;
  type: string;
...
// Domain Event
export interface OperationsManagementEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-ENT-OPERATIONS_MANAGEMENT';
  version: number;
  causationId: string;
}

// Domain Query
export interface OperationsManagementQuery {
  type: string;
...
// Domain State
export interface OperationsManagementState {
  version: number;
  lastUpdated: number;
...
export interface OperationsManagementAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface OperationsManagementHealth {
  organId: 'ORGX-ENT-OPERATIONS_MANAGEMENT';
