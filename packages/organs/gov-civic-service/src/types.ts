/**
 * CivicService Organ — Domain Types
...
// Domain Command
export interface CivicServiceCommand {
  id: string;
  type: string;
...
// Domain Event
export interface CivicServiceEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-GOV-CIVIC_SERVICE';
  version: number;
  causationId: string;
}

// Domain Query
export interface CivicServiceQuery {
  type: string;
...
export interface CivicServiceAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface CivicServiceHealth {
  organId: 'ORGX-GOV-CIVIC_SERVICE';
