/**
 * HealthRecords Organ — Domain Types
...
// Domain Command
export interface HealthRecordsCommand {
  id: string;
  type: string;
...
// Domain Event
export interface HealthRecordsEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-HLT-HEALTH_RECORDS';
  version: number;
  causationId: string;
}
...
export interface HealthRecordsAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface HealthRecordsHealth {
  organId: 'ORGX-HLT-HEALTH_RECORDS';
