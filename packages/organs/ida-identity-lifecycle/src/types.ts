/**
 * IdentityLifecycle Organ — Domain Types
...
export interface IdentityLifecycleEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-IDA-IDENTITY_LIFECYCLE';
  version: number;
  causationId: string;
}

// Domain Query
export interface IdentityLifecycleQuery {
  type: string;
...
export interface IdentityLifecycleAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface IdentityLifecycleHealth {
  organId: 'ORGX-IDA-IDENTITY_LIFECYCLE';
