/**
 * OrderManagement Organ — Domain Types
...
// Domain Command
export interface OrderManagementCommand {
  id: string;
  type: string;
...
// Domain Event
export interface OrderManagementEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-COM-ORDER_MANAGEMENT';
  version: number;
  causationId: string;
}
...
export interface OrderManagementAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface OrderManagementHealth {
  organId: 'ORGX-COM-ORDER_MANAGEMENT';
