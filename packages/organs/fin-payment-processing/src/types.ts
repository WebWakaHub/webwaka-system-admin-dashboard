/**
 * PaymentProcessing Organ — Domain Types
...
// Domain Command
export interface PaymentProcessingCommand {
  id: string;
  type: string;
...
export interface PaymentProcessingEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-FIN-PAYMENT_PROCESSING';
  version: number;
  causationId: string;
}

// Domain Query
export interface PaymentProcessingQuery {
  type: string;
...
export interface PaymentProcessingAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface PaymentProcessingHealth {
  organId: 'ORGX-FIN-PAYMENT_PROCESSING';
