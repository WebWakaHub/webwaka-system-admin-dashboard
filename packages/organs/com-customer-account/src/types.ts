/**
 * CustomerAccount Organ — Domain Types
...
// Domain Command
export interface CustomerAccountCommand {
  id: string;
  type: string;
...
export interface CustomerAccountEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-COM-CUSTOMER_ACCOUNT';
  version: number;
  causationId: string;
}
...
export interface CustomerAccountAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface CustomerAccountHealth {
  organId: 'ORGX-COM-CUSTOMER_ACCOUNT';
