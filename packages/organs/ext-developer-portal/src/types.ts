/**
 * DeveloperPortal Organ — Domain Types
...
// Domain Command
export interface DeveloperPortalCommand {
  id: string;
  type: string;
...
// Domain Event
export interface DeveloperPortalEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-EXT-DEVELOPER_PORTAL';
  version: number;
  causationId: string;
}
...
export interface DeveloperPortalAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface DeveloperPortalHealth {
  organId: 'ORGX-EXT-DEVELOPER_PORTAL';
