/**
 * MetricsCollection Organ — Domain Types
...
// Domain Command
export interface MetricsCollectionCommand {
  id: string;
  type: string;
...
export interface MetricsCollectionEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-IN-METRICS_COLLECTION';
  version: number;
  causationId: string;
}

// Domain Query
export interface MetricsCollectionQuery {
  type: string;
...
export interface MetricsCollectionAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface MetricsCollectionHealth {
  organId: 'ORGX-IN-METRICS_COLLECTION';
