/**
 * PerformanceManagement Organ — Domain Types
...
// Domain Command
export interface PerformanceManagementCommand {
  id: string;
  type: string;
...
// Domain Event
export interface PerformanceManagementEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-ENT-PERFORMANCE_MANAGEMENT';
  version: number;
  causationId: string;
}

// Domain Query
export interface PerformanceManagementQuery {
  type: string;
...
export interface PerformanceManagementAIInference {
  provider: string;
  model: string;
