/**
 * InventoryControl Organ — Domain Types
...
// Domain Command
export interface InventoryControlCommand {
  id: string;
  type: string;
...
// Domain Event
export interface InventoryControlEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-LOG-INVENTORY_CONTROL';
  version: number;
  causationId: string;
}

// Domain Query
export interface InventoryControlQuery {
  type: string;
...
export interface InventoryControlAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface InventoryControlHealth {
  organId: 'ORGX-LOG-INVENTORY_CONTROL';
