...
// Domain Command
export interface ApiGatewayCommand {
  id: string;
  type: string;
...
export interface ApiGatewayEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-EXT-API_GATEWAY';
  version: number;
  causationId: string;
}

// Domain Query
export interface ApiGatewayQuery {
  type: string;
...
export interface ApiGatewayAIInference {
  provider: string;
  model: string;
