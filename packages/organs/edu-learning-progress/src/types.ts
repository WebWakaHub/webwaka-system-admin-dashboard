/**
 * LearningProgress Organ — Domain Types
...
export interface LearningProgressEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-EDU-LEARNING_PROGRESS';
  version: number;
  causationId: string;
}
...
export interface LearningProgressAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface LearningProgressHealth {
  organId: 'ORGX-EDU-LEARNING_PROGRESS';
