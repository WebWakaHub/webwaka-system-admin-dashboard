/**
 * PatientManagement Organ — Domain Types
...
// Domain Command
export interface PatientManagementCommand {
  id: string;
  type: string;
...
// Domain Event
export interface PatientManagementEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-HLT-PATIENT_MANAGEMENT';
  version: number;
  causationId: string;
}

// Domain Query
export interface PatientManagementQuery {
  type: string;
...
export interface PatientManagementAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface PatientManagementHealth {
  organId: 'ORGX-HLT-PATIENT_MANAGEMENT';
