/**
 * ContentManagement Organ — Domain Types
...
// Domain Command
export interface ContentManagementCommand {
  id: string;
  type: string;
...
// Domain Event
export interface ContentManagementEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-MED-CONTENT_MANAGEMENT';
  version: number;
  causationId: string;
}
...
export interface ContentManagementAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface ContentManagementHealth {
  organId: 'ORGX-MED-CONTENT_MANAGEMENT';
