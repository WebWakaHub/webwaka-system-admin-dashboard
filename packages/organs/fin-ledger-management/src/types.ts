/**
 * LedgerManagement Organ — Domain Types
...
// Domain Command
export interface LedgerManagementCommand {
  id: string;
  type: string;
...
// Domain Event
export interface LedgerManagementEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-FIN-LEDGER_MANAGEMENT';
  version: number;
  causationId: string;
}

// Domain Query
export interface LedgerManagementQuery {
  type: string;
...
// Domain State
export interface LedgerManagementState {
  version: number;
  lastUpdated: number;
...
export interface LedgerManagementAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface LedgerManagementHealth {
  organId: 'ORGX-FIN-LEDGER_MANAGEMENT';
