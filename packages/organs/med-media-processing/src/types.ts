/**
 * MediaProcessing Organ — Domain Types
...
// Domain Command
export interface MediaProcessingCommand {
  id: string;
  type: string;
...
export interface MediaProcessingEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-MED-MEDIA_PROCESSING';
  version: number;
  causationId: string;
}

// Domain Query
export interface MediaProcessingQuery {
  type: string;
...
export interface MediaProcessingAIInference {
  provider: string;
  model: string;
