/**
 * CourseManagement Organ — Domain Types
...
// Domain Command
export interface CourseManagementCommand {
  id: string;
  type: string;
...
export interface CourseManagementEvent {
  ...
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-EDU-COURSE_MANAGEMENT';
  version: number;
  causationId: string;
}
...
export interface CourseManagementAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface CourseManagementHealth {
  organId: 'ORGX-EDU-COURSE_MANAGEMENT';
