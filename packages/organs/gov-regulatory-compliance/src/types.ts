/**
 * RegulatoryCompliance Organ — Domain Types
...
// Domain Command
export interface RegulatoryComplianceCommand {
  id: string;
  type: string;
...
// Domain Event
export interface RegulatoryComplianceEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-GOV-REGULATORY_COMPLIANCE';
  version: number;
  causationId: string;
}

// Domain Query
export interface RegulatoryComplianceQuery {
  type: string;
...
// Domain State
export interface RegulatoryComplianceState {
  version: number;
  lastUpdated: number;
...
export interface RegulatoryComplianceAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface RegulatoryComplianceHealth {
  organId: 'ORGX-GOV-REGULATORY_COMPLIANCE';
