/**
 * ModelServing Organ — Domain Types
...
// Domain Command
export interface ModelServingCommand {
  id: string;
  type: string;
...
// Domain Event
export interface ModelServingEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-AI-MODEL_SERVING';
  version: number;
  causationId: string;
}

// Domain Query
export interface ModelServingQuery {
  type: string;
...
export interface ModelServingAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface ModelServingHealth {
  organId: 'ORGX-AI-MODEL_SERVING';
