/**
 * PolicyEngine Organ — Domain Types
...
// Domain Command
export interface PolicyEngineCommand {
  id: string;
  type: string;
...
// Domain Event
export interface PolicyEngineEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-CFG-POLICY_ENGINE';
  version: number;
  causationId: string;
}

// Domain Query
export interface PolicyEngineQuery {
  type: string;
...
export interface PolicyEngineAIInference {
  provider: string;
  model: string;
