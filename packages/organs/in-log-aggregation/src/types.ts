/**
 * LogAggregation Organ — Domain Types
...
// Domain Command
export interface LogAggregationCommand {
  id: string;
  type: string;
...
// Domain Event
export interface LogAggregationEvent {
  id: string;
  type: string;
  payload: Record<string, unknown>;
  timestamp: number;
  organId: 'ORGX-IN-LOG_AGGREGATION';
  version: number;
  causationId: string;
}
...
// Domain State
export interface LogAggregationState {
  version: number;
  lastUpdated: number;
...
export interface LogAggregationAIInference {
  provider: string;
  model: string;
...
// Organ Health
export interface LogAggregationHealth {
  organId: 'ORGX-IN-LOG_AGGREGATION';
