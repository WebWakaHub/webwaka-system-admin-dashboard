/**
 * SchedulerExecutor Entity — Unit Tests
 * Organelle: ORG-ES-SCHEDULER_EXECUTOR-v0.1.0
 */

import { SchedulerExecutorEntity } from "../src/scheduler-executor-entity";
import { SchedulerExecutorConfig, SchedulerExecutorState, SchedulerExecutorCommand } from "../src/types";

describe("SchedulerExecutorEntity", () => {
  let entity: SchedulerExecutorEntity;
  const config: SchedulerExecutorConfig = {
    id: "test-scheduler-executor-001",
    name: "Test SchedulerExecutor",
    maxConcurrency: 5,
    timeoutMs: 30000,
  ...
  beforeEach(() => {
    entity = new SchedulerExecutorEntity(config);
  });

  describe("initialization", () => {
    it("should initialize with IDLE state", () => {
      expect(entity.getState()).toBe(SchedulerExecutorState.IDLE);
    });

    it("should have the correct ID", () => {
      expect(entity.getId()).toBe("test-scheduler-executor-001");
    });
  ...
  describe("state transitions", () => {
    it("should transition from IDLE to PROCESSING", () => {
      entity.setState(SchedulerExecutorState.PROCESSING);
      expect(entity.getState()).toBe(SchedulerExecutorState.PROCESSING);
    });

    it("should reject invalid transitions", () => {
      expect(() => entity.setState(SchedulerExecutorState.COMPLETED)).toThrow("Invalid state transition");
    });
    ...
    it("should not allow transition from TERMINATED", () => {
      entity.setState(SchedulerExecutorState.TERMINATED);
      expect(() => entity.setState(SchedulerExecutorState.IDLE)).toThrow();
    });
  });
  ...
  describe("snapshot", () => {
    it("should produce a valid snapshot", () => {
      ...
      expect(snapshot.id).toBe("test-scheduler-executor-001");
      expect(snapshot.state).toBe(SchedulerExecutorState.IDLE);
