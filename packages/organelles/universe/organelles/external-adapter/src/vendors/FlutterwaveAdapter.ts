/**
 * ORG-EI-EXTERNAL_ADAPTER-v0.1.0 — FlutterwaveAdapter
...
export class FlutterwaveAdapter implements IVendorAdapter {
  readonly vendorId = 'flutterwave';
  ...
  async execute<T, R>(operation: string, payload: T, timeout: number): Promise<R> {
    ...
    try {
      const url = `${this.config.baseUrl}/v3/${operation}`;
