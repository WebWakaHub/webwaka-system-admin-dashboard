/**
 * DiscoveryRegistry Orchestrator — Integration Tests
 * Organelle: ORG-OD-DISCOVERY_REGISTRY-v0.1.0
 */

import { DiscoveryRegistryOrchestrator } from "../src/discovery-registry-orchestrator";
import { DiscoveryRegistryConfig, DiscoveryRegistryState, DiscoveryRegistryCommand } from "../src/types";
import { InMemoryDiscoveryRegistryStorage } from "../src/storage-interface";
import { DiscoveryRegistryEventBus } from "../src/event-interface";

describe("DiscoveryRegistryOrchestrator", () => {
  let orchestrator: DiscoveryRegistryOrchestrator;
  let storage: InMemoryDiscoveryRegistryStorage;
  let eventBus: DiscoveryRegistryEventBus;
  ...
  beforeEach(() => {
    storage = new InMemoryDiscoveryRegistryStorage();
    eventBus = new DiscoveryRegistryEventBus();
    orchestrator = new DiscoveryRegistryOrchestrator(config, storage, eventBus);
  });

  it("should initialize in IDLE state", () => {
    expect(orchestrator.getState()).toBe(DiscoveryRegistryState.IDLE);
  });
  ...
  it("should return valid telemetry", () => {
    ...
    expect(telemetry.state).toBe(DiscoveryRegistryState.IDLE);
  });
