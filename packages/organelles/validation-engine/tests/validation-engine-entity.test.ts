/**
 * ValidationEngine Entity — Unit Tests
 * Organelle: ORG-FV-VALIDATION_ENGINE-v0.1.0
 */

import { ValidationEngineEntity } from "../src/validation-engine-entity";
import { ValidationEngineConfig, ValidationEngineState, ValidationEngineCommand } from "../src/types";

describe("ValidationEngineEntity", () => {
  let entity: ValidationEngineEntity;
  const config: ValidationEngineConfig = {
    id: "test-validation-engine-001",
    name: "Test ValidationEngine",
    maxConcurrency: 5,
    timeoutMs: 30000,
  ...
  beforeEach(() => {
    entity = new ValidationEngineEntity(config);
  });

  describe("initialization", () => {
    it("should initialize with IDLE state", () => {
      expect(entity.getState()).toBe(ValidationEngineState.IDLE);
    });

    it("should have the correct ID", () => {
      expect(entity.getId()).toBe("test-validation-engine-001");
    });
  ...
  describe("state transitions", () => {
    it("should transition from IDLE to PROCESSING", () => {
      entity.setState(ValidationEngineState.PROCESSING);
      expect(entity.getState()).toBe(ValidationEngineState.PROCESSING);
    });

    it("should reject invalid transitions", () => {
      expect(() => entity.setState(ValidationEngineState.COMPLETED)).toThrow("Invalid state transition");
    });
    ...
    it("should not allow transition from TERMINATED", () => {
      entity.setState(ValidationEngineState.TERMINATED);
      expect(() => entity.setState(ValidationEngineState.IDLE)).toThrow();
    });
  });

  describe("command execution", () => {
    it("should execute CREATE command successfully", () => {
      const command: ValidationEngineCommand = {
        type: "CREATE",
        payload: { name: "test" },
  ...
  describe("snapshot", () => {
    it("should produce a valid snapshot", () => {
      ...
      expect(snapshot.id).toBe("test-validation-engine-001");
      expect(snapshot.state).toBe(ValidationEngineState.IDLE);
