/**
 * AuditLogger Entity — Unit Tests
 * Organelle: ORG-LG-AUDIT_LOGGER-v0.1.0
 */

import { AuditLoggerEntity } from "../src/audit-logger-entity";
import { AuditLoggerConfig, AuditLoggerState, AuditLoggerCommand } from "../src/types";

describe("AuditLoggerEntity", () => {
  let entity: AuditLoggerEntity;
  const config: AuditLoggerConfig = {
    id: "test-audit-logger-001",
    name: "Test AuditLogger",
    maxConcurrency: 5,
    timeoutMs: 30000,
  ...
  beforeEach(() => {
    entity = new AuditLoggerEntity(config);
  });

  describe("initialization", () => {
    it("should initialize with IDLE state", () => {
      expect(entity.getState()).toBe(AuditLoggerState.IDLE);
    });

    it("should have the correct ID", () => {
      expect(entity.getId()).toBe("test-audit-logger-001");
    });
  ...
  describe("state transitions", () => {
    it("should transition from IDLE to PROCESSING", () => {
      entity.setState(AuditLoggerState.PROCESSING);
      expect(entity.getState()).toBe(AuditLoggerState.PROCESSING);
    });

    it("should reject invalid transitions", () => {
      expect(() => entity.setState(AuditLoggerState.COMPLETED)).toThrow("Invalid state transition");
    });
    ...
    it("should not allow transition from TERMINATED", () => {
      entity.setState(AuditLoggerState.TERMINATED);
      expect(() => entity.setState(AuditLoggerState.IDLE)).toThrow();
    });
  });

  describe("command execution", () => {
    it("should execute CREATE command successfully", () => {
      const command: AuditLoggerCommand = {
        type: "CREATE",
        payload: { name: "test" },
  ...
  describe("snapshot", () => {
    it("should produce a valid snapshot", () => {
      ...
      expect(snapshot.id).toBe("test-audit-logger-001");
      expect(snapshot.state).toBe(AuditLoggerState.IDLE);
