/**
 * CognitivePort Entity — Unit Tests
 * Organelle: ORGN-AI-COGNITIVE_PORT-v0.1.0
 */

import { CognitivePortEntity } from "../src/cognitive-port-entity";
import { CognitivePortConfig, CognitivePortState, CognitivePortCommand } from "../src/types";

describe("CognitivePortEntity", () => {
  let entity: CognitivePortEntity;
  const config: CognitivePortConfig = {
    id: "test-cognitive-port-001",
    name: "Test CognitivePort",
    maxConcurrency: 5,
    timeoutMs: 30000,
  ...
  beforeEach(() => {
    entity = new CognitivePortEntity(config);
  });

  describe("initialization", () => {
    it("should initialize with IDLE state", () => {
      expect(entity.getState()).toBe(CognitivePortState.IDLE);
    });

    it("should have the correct ID", () => {
      expect(entity.getId()).toBe("test-cognitive-port-001");
    });
  ...
  describe("state transitions", () => {
    it("should transition from IDLE to PROCESSING", () => {
      entity.setState(CognitivePortState.PROCESSING);
      expect(entity.getState()).toBe(CognitivePortState.PROCESSING);
    });

    it("should reject invalid transitions", () => {
      expect(() => entity.setState(CognitivePortState.COMPLETED)).toThrow("Invalid state transition");
    });
    ...
    it("should not allow transition from TERMINATED", () => {
      entity.setState(CognitivePortState.TERMINATED);
      expect(() => entity.setState(CognitivePortState.IDLE)).toThrow();
    });
  });

  describe("command execution", () => {
    it("should execute CREATE command successfully", () => {
      const command: CognitivePortCommand = {
        type: "CREATE",
        payload: { name: "test" },
  ...
  describe("snapshot", () => {
    it("should produce a valid snapshot", () => {
      ...
      expect(snapshot.id).toBe("test-cognitive-port-001");
      expect(snapshot.state).toBe(CognitivePortState.IDLE);
