/**
 * PolicyDefinition Orchestrator — Integration Tests
 * Organelle: ORG-CP-POLICY_DEFINITION-v0.1.0
 */

import { PolicyDefinitionOrchestrator } from "../src/policy-definition-orchestrator";
import { PolicyDefinitionConfig, PolicyDefinitionState, PolicyDefinitionCommand } from "../src/types";
import { InMemoryPolicyDefinitionStorage } from "../src/storage-interface";
import { PolicyDefinitionEventBus } from "../src/event-interface";

describe("PolicyDefinitionOrchestrator", () => {
  let orchestrator: PolicyDefinitionOrchestrator;
  let storage: InMemoryPolicyDefinitionStorage;
  let eventBus: PolicyDefinitionEventBus;
  ...
  beforeEach(() => {
    storage = new InMemoryPolicyDefinitionStorage();
    eventBus = new PolicyDefinitionEventBus();
    orchestrator = new PolicyDefinitionOrchestrator(config, storage, eventBus);
  });

  it("should initialize in IDLE state", () => {
    expect(orchestrator.getState()).toBe(PolicyDefinitionState.IDLE);
  });
  ...
  it("should return valid telemetry", () => {
    ...
    expect(telemetry.state).toBe(PolicyDefinitionState.IDLE);
  });
