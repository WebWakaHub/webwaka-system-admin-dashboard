/**
 * CompositionModeler Entity — Unit Tests
 * Organelle: ORG-CM-COMPOSITION_MODELER-v0.1.0
 */

import { CompositionModelerEntity } from "../src/composition-modeler-entity";
import { CompositionModelerConfig, CompositionModelerState, CompositionModelerCommand } from "../src/types";

describe("CompositionModelerEntity", () => {
  let entity: CompositionModelerEntity;
  const config: CompositionModelerConfig = {
    id: "test-composition-modeler-001",
    name: "Test CompositionModeler",
    maxConcurrency: 5,
    timeoutMs: 30000,
  ...
  beforeEach(() => {
    entity = new CompositionModelerEntity(config);
  });

  describe("initialization", () => {
    it("should initialize with IDLE state", () => {
      expect(entity.getState()).toBe(CompositionModelerState.IDLE);
    });

    it("should have the correct ID", () => {
      expect(entity.getId()).toBe("test-composition-modeler-001");
    });
  ...
  describe("state transitions", () => {
    it("should transition from IDLE to PROCESSING", () => {
      entity.setState(CompositionModelerState.PROCESSING);
      expect(entity.getState()).toBe(CompositionModelerState.PROCESSING);
    });

    it("should reject invalid transitions", () => {
      expect(() => entity.setState(CompositionModelerState.COMPLETED)).toThrow("Invalid state transition");
    });
    ...
    it("should not allow transition from TERMINATED", () => {
      entity.setState(CompositionModelerState.TERMINATED);
      expect(() => entity.setState(CompositionModelerState.IDLE)).toThrow();
    });
  });
  ...
  describe("snapshot", () => {
    it("should produce a valid snapshot", () => {
      ...
      expect(snapshot.id).toBe("test-composition-modeler-001");
      expect(snapshot.state).toBe(CompositionModelerState.IDLE);
