/**
 * Monitor — Cell Entity Implementation
...
const DEFAULT_CONFIG: MonitorConfig = {
  ...
  offlineStorageKey: 'cel_monitor_offline_queue',
...
export class Monitor {
   ...
   * Follows: MetricCollector → HealthChecker → AlertManager → DiagnosticReporter
   */
  async execute(command: MonitorCommand, context: ExecutionContext): Promise<MonitorResult> {
    ...
    try {
      // Step 1: MetricCollector — Intake and initialization
      ...
      if (!validated.valid) {
        throw new MonitorValidationError(validated.errors);
      }

      // Step 2: HealthChecker — Processing
      ...
      // Step 4: DiagnosticReporter — Delivery
   ...
   */
  async executeOffline(command: MonitorCommand, context: ExecutionContext): Promise<MonitorResult> {
  ...
  private async validate(command: MonitorCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: MonitorCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<MonitorResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: MonitorCommand, context: ExecutionContext): Promise<MonitorResult> {
  ...
  private canRetry(command: MonitorCommand): boolean {
