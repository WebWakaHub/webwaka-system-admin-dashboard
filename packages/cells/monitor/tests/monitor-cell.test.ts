...
import { Monitor, MonitorValidationError } from '../src/monitor-cell';
import { MonitorCommand, ExecutionContext, CellState } from '../src/types';

describe('Monitor', () => {
  let cell: Monitor;

  const mockCommand: MonitorCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(MonitorValidationError);
    });
