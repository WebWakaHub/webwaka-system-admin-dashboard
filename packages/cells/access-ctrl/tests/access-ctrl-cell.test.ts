/**
 * AccessController — Cell Entity Tests
...
import { AccessController, AccessControllerValidationError } from '../src/access-ctrl-cell';
import { AccessControllerCommand, ExecutionContext, CellState } from '../src/types';

describe('AccessController', () => {
  let cell: AccessController;

  const mockCommand: AccessControllerCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(AccessControllerValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new AccessController();
