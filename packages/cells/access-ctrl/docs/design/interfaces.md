# AccessController — Interface Design
...
export interface IAccessController {
  execute(command: AccessControllerCommand, context: ExecutionContext): Promise<AccessControllerResult>;
  executeOffline(command: AccessControllerCommand, context: ExecutionContext): Promise<OfflineReceipt>;
...
```typescript
export interface IPermissionDefiner {
...
```typescript
export interface AccessControllerEvents {
