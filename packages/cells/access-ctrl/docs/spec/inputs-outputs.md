# AccessController — Inputs & Outputs Specification
...
|:-----|:-----|:-------|:------------|
| `input.command` | `AccessControllerCommand` | External caller | Primary command/request payload |
| `input.context` | `ExecutionContext` | Runtime | Execution context with identity, tenant, locale |
| `input.config` | `AccessControllerConfig` | Configuration store | Cell-specific configuration |
...
|:-----|:-----|:------------|:------------|
| `output.result` | `AccessControllerResult` | Caller | Processed result payload |
