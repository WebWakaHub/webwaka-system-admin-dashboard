/**
 * StreamingCell — Cell Entity Tests
...
import { StreamingCell, StreamingCellValidationError } from '../src/ai-streaming-cell-cell';
import { StreamingCellCommand, ExecutionContext, CellState } from '../src/types';

describe('StreamingCell', () => {
  let cell: StreamingCell;

  const mockCommand: StreamingCellCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(StreamingCellValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new StreamingCell();
