...
const DEFAULT_CONFIG: EventDispatcherConfig = {
  ...
  offlineStorageKey: 'cel_eventdispatch_offline_queue',
...
export class EventDispatcher {
   ...
   * Follows: EventCapture → EventValidator → EventRouter → EventDelivery
   */
  async execute(command: EventDispatcherCommand, context: ExecutionContext): Promise<EventDispatcherResult> {
    ...
    try {
      // Step 1: EventCapture — Intake and initialization
      ...
      if (!validated.valid) {
        throw new EventDispatcherValidationError(validated.errors);
      }

      // Step 2: EventValidator — Processing
      ...
      // Step 3: EventRouter — Routing
      const routed = await this.route(processed, context);

      // Step 4: EventDelivery — Delivery
   ...
   */
  async executeOffline(command: EventDispatcherCommand, context: ExecutionContext): Promise<EventDispatcherResult> {
  ...
  private async validate(command: EventDispatcherCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: EventDispatcherCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<EventDispatcherResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: EventDispatcherCommand, context: ExecutionContext): Promise<EventDispatcherResult> {
  ...
  private canRetry(command: EventDispatcherCommand): boolean {
...
export class EventDispatcherValidationError extends Error {
  constructor(public readonly errors: any[]) {
    super(`Validation failed: ${errors.map(e => e.message).join(', ')}`);
    this.name = 'EventDispatcherValidationError';
  }
}
