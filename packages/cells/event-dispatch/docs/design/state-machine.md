# EventDispatcher — State Machine Design
...
| `VALIDATING` | Input is being validated by EventValidator | → PROCESSING, → ERROR |
| `PROCESSING` | Core logic executing via EventRouter and EventDelivery | → COMPLETING, → ERROR |
