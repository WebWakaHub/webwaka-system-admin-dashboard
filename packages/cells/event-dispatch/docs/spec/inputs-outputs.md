# EventDispatcher — Inputs & Outputs Specification
...
|:-----|:-----|:-------|:------------|
| `input.command` | `EventDispatcherCommand` | External caller | Primary command/request payload |
| `input.context` | `ExecutionContext` | Runtime | Execution context with identity, tenant, locale |
| `input.config` | `EventDispatcherConfig` | Configuration store | Cell-specific configuration |
...
|:-----|:-----|:------------|:------------|
| `output.result` | `EventDispatcherResult` | Caller | Processed result payload |
