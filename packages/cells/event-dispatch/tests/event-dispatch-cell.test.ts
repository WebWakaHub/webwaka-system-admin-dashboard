/**
 * EventDispatcher — Cell Entity Tests
...
import { EventDispatcher, EventDispatcherValidationError } from '../src/event-dispatch-cell';
import { EventDispatcherCommand, ExecutionContext, CellState } from '../src/types';

describe('EventDispatcher', () => {
  let cell: EventDispatcher;

  const mockCommand: EventDispatcherCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(EventDispatcherValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new EventDispatcher();
