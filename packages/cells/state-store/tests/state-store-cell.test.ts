...
import { StateStore, StateStoreValidationError } from '../src/state-store-cell';
import { StateStoreCommand, ExecutionContext, CellState } from '../src/types';

describe('StateStore', () => {
  let cell: StateStore;

  const mockCommand: StateStoreCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(StateStoreValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new StateStore();
