/**
 * CommandProcessor — Cell Entity Tests
...
import { CommandProcessor, CommandProcessorValidationError } from '../src/cmd-process-cell';
import { CommandProcessorCommand, ExecutionContext, CellState } from '../src/types';

describe('CommandProcessor', () => {
  let cell: CommandProcessor;

  const mockCommand: CommandProcessorCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(CommandProcessorValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new CommandProcessor();
