/**
 * CommandProcessor — Cell Entity Implementation
 * Cell: CEL-CMDPROCESS-v0.1.0
 * Category: Workflow & Orchestration
 * 
 * This is the core cell implementation that composes organelles
 * from the Workflow & Orchestration category into a reusable capability unit.
 * 
 * Doctrine Compliance:
...
const DEFAULT_CONFIG: CommandProcessorConfig = {
  maxRetries: 3,
  timeoutMs: 30000, // Nigeria-first: 30s default
  offlineStorageKey: 'cel_cmdprocess_offline_queue',
...
export class CommandProcessor {
   ...
   * Follows: CommandGateway → CommandValidator → CommandRouter → CommandExecutor
   */
  async execute(command: CommandProcessorCommand, context: ExecutionContext): Promise<CommandProcessorResult> {
    ...
    try {
      // Step 1: CommandGateway — Intake and initialization
      ...
      if (!validated.valid) {
        throw new CommandProcessorValidationError(validated.errors);
      }

      // Step 2: CommandValidator — Processing
      ...
      // Step 3: CommandRouter — Routing
      const routed = await this.route(processed, context);

      // Step 4: CommandExecutor — Delivery
   ...
   */
  async executeOffline(command: CommandProcessorCommand, context: ExecutionContext): Promise<CommandProcessorResult> {
  ...
  private async validate(command: CommandProcessorCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: CommandProcessorCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<CommandProcessorResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: CommandProcessorCommand, context: ExecutionContext): Promise<CommandProcessorResult> {
  ...
  private canRetry(command: CommandProcessorCommand): boolean {
...
export class CommandProcessorValidationError extends Error {
  constructor(public readonly errors: any[]) {
    super(`Validation failed: ${errors.map(e => e.message).join(', ')}`);
    this.name = 'CommandProcessorValidationError';
  }
}
