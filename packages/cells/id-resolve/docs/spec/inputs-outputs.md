# IdentityResolver — Inputs & Outputs Specification
...
|:-----|:-----|:-------|:------------|
| `input.command` | `IdentityResolverCommand` | External caller | Primary command/request payload |
| `input.context` | `ExecutionContext` | Runtime | Execution context with identity, tenant, locale |
| `input.config` | `IdentityResolverConfig` | Configuration store | Cell-specific configuration |
...
|:-----|:-----|:------------|:------------|
| `output.result` | `IdentityResolverResult` | Caller | Processed result payload |
