# IdentityResolver — Interface Design
...
export interface IIdentityResolver {
  execute(command: IdentityResolverCommand, context: ExecutionContext): Promise<IdentityResolverResult>;
  executeOffline(command: IdentityResolverCommand, context: ExecutionContext): Promise<OfflineReceipt>;
...
```typescript
export interface IdentityResolverEvents {
