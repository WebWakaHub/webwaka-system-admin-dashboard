/**
 * IdentityResolver — Cell Entity Tests
...
import { IdentityResolver, IdentityResolverValidationError } from '../src/id-resolve-cell';
import { IdentityResolverCommand, ExecutionContext, CellState } from '../src/types';

describe('IdentityResolver', () => {
  let cell: IdentityResolver;

  const mockCommand: IdentityResolverCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(IdentityResolverValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new IdentityResolver();
