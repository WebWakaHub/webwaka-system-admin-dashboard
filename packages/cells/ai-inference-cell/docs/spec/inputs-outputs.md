# InferenceCell — Inputs & Outputs Specification
...
|:-----|:-----|:-------|:------------|
| `input.command` | `InferenceCellCommand` | External caller | Primary command/request payload |
| `input.context` | `ExecutionContext` | Runtime | Execution context with identity, tenant, locale |
| `input.config` | `InferenceCellConfig` | Configuration store | Cell-specific configuration |
...
|:-----|:-----|:------------|:------------|
| `output.result` | `InferenceCellResult` | Caller | Processed result payload |
