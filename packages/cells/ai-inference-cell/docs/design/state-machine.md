# InferenceCell — State Machine Design
...
| `VALIDATING` | Input is being validated by ModelInvoker | → PROCESSING, → ERROR |
| `PROCESSING` | Core logic executing via ResultValidator and InferenceCache | → COMPLETING, → ERROR |
