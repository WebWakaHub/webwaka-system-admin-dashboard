/**
 * InferenceCell — Cell Entity Tests
...
import { InferenceCell, InferenceCellValidationError } from '../src/ai-inference-cell-cell';
import { InferenceCellCommand, ExecutionContext, CellState } from '../src/types';

describe('InferenceCell', () => {
  let cell: InferenceCell;

  const mockCommand: InferenceCellCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(InferenceCellValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new InferenceCell();
