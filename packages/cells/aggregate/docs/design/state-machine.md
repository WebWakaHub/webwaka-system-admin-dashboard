# Aggregator — State Machine Design
...
| `VALIDATING` | Input is being validated by DataMerger | → PROCESSING, → ERROR |
| `PROCESSING` | Core logic executing via DataReducer and AggregateEmitter | → COMPLETING, → ERROR |
