/**
 * Aggregator — Cell Entity Implementation
...
const DEFAULT_CONFIG: AggregatorConfig = {
  ...
  offlineStorageKey: 'cel_aggregate_offline_queue',
...
export class Aggregator {
   ...
   * Follows: DataCollector → DataMerger → DataReducer → AggregateEmitter
   */
  async execute(command: AggregatorCommand, context: ExecutionContext): Promise<AggregatorResult> {
    ...
    try {
      // Step 1: DataCollector — Intake and initialization
      ...
      if (!validated.valid) {
        throw new AggregatorValidationError(validated.errors);
      }

      // Step 2: DataMerger — Processing
      ...
      // Step 3: DataReducer — Routing
      const routed = await this.route(processed, context);

      // Step 4: AggregateEmitter — Delivery
   ...
   */
  async executeOffline(command: AggregatorCommand, context: ExecutionContext): Promise<AggregatorResult> {
  ...
  private async validate(command: AggregatorCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: AggregatorCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<AggregatorResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: AggregatorCommand, context: ExecutionContext): Promise<AggregatorResult> {
  ...
  private canRetry(command: AggregatorCommand): boolean {
...
export class AggregatorValidationError extends Error {
