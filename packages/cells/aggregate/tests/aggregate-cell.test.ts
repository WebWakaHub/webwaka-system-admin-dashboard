...
import { Aggregator, AggregatorValidationError } from '../src/aggregate-cell';
import { AggregatorCommand, ExecutionContext, CellState } from '../src/types';

describe('Aggregator', () => {
  let cell: Aggregator;

  const mockCommand: AggregatorCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(AggregatorValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new Aggregator();
