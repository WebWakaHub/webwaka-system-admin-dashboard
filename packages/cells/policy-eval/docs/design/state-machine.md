# PolicyEvaluator — State Machine Design
...
| `VALIDATING` | Input is being validated by PolicyParser | → PROCESSING, → ERROR |
| `PROCESSING` | Core logic executing via PolicyEngine and PolicyEnforcer | → COMPLETING, → ERROR |
