/**
 * PolicyEvaluator — Cell Entity Tests
...
import { PolicyEvaluator, PolicyEvaluatorValidationError } from '../src/policy-eval-cell';
import { PolicyEvaluatorCommand, ExecutionContext, CellState } from '../src/types';

describe('PolicyEvaluator', () => {
  let cell: PolicyEvaluator;

  const mockCommand: PolicyEvaluatorCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(PolicyEvaluatorValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new PolicyEvaluator();
