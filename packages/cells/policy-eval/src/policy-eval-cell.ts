...
const DEFAULT_CONFIG: PolicyEvaluatorConfig = {
  ...
  offlineStorageKey: 'cel_policyeval_offline_queue',
...
export class PolicyEvaluator {
   ...
   * Follows: PolicyLoader → PolicyParser → PolicyEngine → PolicyEnforcer
   */
  async execute(command: PolicyEvaluatorCommand, context: ExecutionContext): Promise<PolicyEvaluatorResult> {
    ...
    try {
      // Step 1: PolicyLoader — Intake and initialization
      ...
      if (!validated.valid) {
        throw new PolicyEvaluatorValidationError(validated.errors);
      }

      // Step 2: PolicyParser — Processing
      ...
      // Step 3: PolicyEngine — Routing
      const routed = await this.route(processed, context);

      // Step 4: PolicyEnforcer — Delivery
   ...
   */
  async executeOffline(command: PolicyEvaluatorCommand, context: ExecutionContext): Promise<PolicyEvaluatorResult> {
  ...
  private async validate(command: PolicyEvaluatorCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: PolicyEvaluatorCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<PolicyEvaluatorResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: PolicyEvaluatorCommand, context: ExecutionContext): Promise<PolicyEvaluatorResult> {
  ...
  private canRetry(command: PolicyEvaluatorCommand): boolean {
...
export class PolicyEvaluatorValidationError extends Error {
  constructor(public readonly errors: any[]) {
    super(`Validation failed: ${errors.map(e => e.message).join(', ')}`);
    this.name = 'PolicyEvaluatorValidationError';
  }
}
