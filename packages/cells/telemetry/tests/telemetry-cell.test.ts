/**
 * TelemetryCell — Cell Entity Tests
...
import { TelemetryCell, TelemetryCellValidationError } from '../src/telemetry-cell';
import { TelemetryCellCommand, ExecutionContext, CellState } from '../src/types';

describe('TelemetryCell', () => {
  let cell: TelemetryCell;

  const mockCommand: TelemetryCellCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(TelemetryCellValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new TelemetryCell();
