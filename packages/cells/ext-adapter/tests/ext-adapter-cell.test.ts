/**
 * ExternalAdapter — Cell Entity Tests
...
import { ExternalAdapter, ExternalAdapterValidationError } from '../src/ext-adapter-cell';
import { ExternalAdapterCommand, ExecutionContext, CellState } from '../src/types';

describe('ExternalAdapter', () => {
  let cell: ExternalAdapter;

  const mockCommand: ExternalAdapterCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(ExternalAdapterValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new ExternalAdapter();
