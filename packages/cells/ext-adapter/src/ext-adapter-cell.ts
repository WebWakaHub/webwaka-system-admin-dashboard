...
const DEFAULT_CONFIG: ExternalAdapterConfig = {
  maxRetries: 3,
  timeoutMs: 30000, // Nigeria-first: 30s default
  offlineStorageKey: 'cel_extadapter_offline_queue',
...
export class ExternalAdapter {
   ...
   * Follows: ProtocolAdapter → DataTransformer → RequestProxy → IntegrationBridge
   */
  async execute(command: ExternalAdapterCommand, context: ExecutionContext): Promise<ExternalAdapterResult> {
    ...
    try {
      // Step 1: ProtocolAdapter — Intake and initialization
      ...
      if (!validated.valid) {
        throw new ExternalAdapterValidationError(validated.errors);
      }

      // Step 2: DataTransformer — Processing
      ...
      // Step 3: RequestProxy — Routing
      const routed = await this.route(processed, context);

      // Step 4: IntegrationBridge — Delivery
   ...
   */
  async executeOffline(command: ExternalAdapterCommand, context: ExecutionContext): Promise<ExternalAdapterResult> {
  ...
  private async validate(command: ExternalAdapterCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: ExternalAdapterCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<ExternalAdapterResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: ExternalAdapterCommand, context: ExecutionContext): Promise<ExternalAdapterResult> {
  ...
  private canRetry(command: ExternalAdapterCommand): boolean {
...
export class ExternalAdapterValidationError extends Error {
  constructor(public readonly errors: any[]) {
    super(`Validation failed: ${errors.map(e => e.message).join(', ')}`);
    this.name = 'ExternalAdapterValidationError';
  }
}
