# ExternalAdapter — Inputs & Outputs Specification
...
|:-----|:-----|:-------|:------------|
| `input.command` | `ExternalAdapterCommand` | External caller | Primary command/request payload |
| `input.context` | `ExecutionContext` | Runtime | Execution context with identity, tenant, locale |
| `input.config` | `ExternalAdapterConfig` | Configuration store | Cell-specific configuration |
...
|:-----|:-----|:------------|:------------|
| `output.result` | `ExternalAdapterResult` | Caller | Processed result payload |
