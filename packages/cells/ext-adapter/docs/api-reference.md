# ExternalAdapter — API Reference

**Cell:** CEL-EXTADAPTER-v0.1.0

## Classes

### `ExternalAdapter`

The core cell implementation.

#### Constructor

```typescript
new ExternalAdapter(config?: Partial<ExternalAdapterConfig>)
```

#### Methods

| Method | Parameters | Returns | Description |
|:-------|:-----------|:--------|:------------|
| `execute` | `command: ExternalAdapterCommand, context: ExecutionContext` | `Promise<ExternalAdapterResult>` | Execute a command through the cell pipeline |
| `executeOffline` | `command: ExternalAdapterCommand, context: ExecutionContext` | `Promise<ExternalAdapterResult>` | Queue a command for offline execution |
| `sync` | none | `Promise<SyncResult>` | Sync offline queue |
| `getState` | none | `CellState` | Get current cell state |
| `getMetrics` | none | `CellMetric[]` | Get collected metrics |
| `onStateChange` | `handler: (state: CellState) => void` | `Unsubscribe` | Subscribe to state changes |
| `onError` | `handler: (error: Error) => void` | `Unsubscribe` | Subscribe to errors |
| `dispose` | none | `Promise<void>` | Clean up resources |

### `ExternalAdapterOrchestrator`

Manages the cell lifecycle.

#### Methods

| Method | Returns | Description |
|:-------|:--------|:------------|
| `initialize` | `Promise<ExternalAdapter>` | Create and initialize the cell |
| `shutdown` | `Promise<void>` | Gracefully shut down the cell |
| `getCell` | `ExternalAdapter \| null` | Get the managed cell instance |

## Types

See `src/types.ts` for complete type definitions.
