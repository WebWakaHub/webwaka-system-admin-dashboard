# ExternalAdapter — State Machine Design
...
| `VALIDATING` | Input is being validated by DataTransformer | → PROCESSING, → ERROR |
| `PROCESSING` | Core logic executing via RequestProxy and IntegrationBridge | → COMPLETING, → ERROR |
