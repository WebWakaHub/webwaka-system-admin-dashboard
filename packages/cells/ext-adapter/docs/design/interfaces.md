# ExternalAdapter — Interface Design
...
export interface IExternalAdapter {
  execute(command: ExternalAdapterCommand, context: ExecutionContext): Promise<ExternalAdapterResult>;
  executeOffline(command: ExternalAdapterCommand, context: ExecutionContext): Promise<OfflineReceipt>;
...
```typescript
export interface ExternalAdapterEvents {
