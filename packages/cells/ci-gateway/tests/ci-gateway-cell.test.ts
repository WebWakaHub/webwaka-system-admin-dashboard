...
import { CIGateway, CIGatewayValidationError } from '../src/ci-gateway-cell';
import { CIGatewayCommand, ExecutionContext, CellState } from '../src/types';

describe('CIGateway', () => {
  let cell: CIGateway;

  const mockCommand: CIGatewayCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(CIGatewayValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new CIGateway();
