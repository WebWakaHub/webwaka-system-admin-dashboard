...
const DEFAULT_CONFIG: CIGatewayConfig = {
  maxRetries: 3,
  timeoutMs: 30000, // Nigeria-first: 30s default
  offlineStorageKey: 'cel_cigateway_offline_queue',
...
export class CIGateway {
  ...
  private config: CIGatewayConfig;

  constructor(config?: Partial<CIGatewayConfig>) {
    this.config = { ...DEFAULT_CONFIG, ...config };
  }

  /**
   * Execute a command through the cell pipeline.
   * Follows: ChannelReceiver → MessageValidator → ProtocolNormalizer → InteractionRouter
   */
  async execute(command: CIGatewayCommand, context: ExecutionContext): Promise<CIGatewayResult> {
    ...
    try {
      // Step 1: ChannelReceiver — Intake and initialization
      ...
      if (!validated.valid) {
        throw new CIGatewayValidationError(validated.errors);
      }

      // Step 2: MessageValidator — Processing
      ...
      // Step 3: ProtocolNormalizer — Routing
      const routed = await this.route(processed, context);

      // Step 4: InteractionRouter — Delivery
   ...
   */
  async executeOffline(command: CIGatewayCommand, context: ExecutionContext): Promise<CIGatewayResult> {
  ...
  private async validate(command: CIGatewayCommand): Promise<ValidationResult> {
    const errors: any[] = [];
  ...
  private async process(command: CIGatewayCommand, context: ExecutionContext): Promise<Record<string, unknown>> {
    return {
      commandId: command.id,
  ...
  private async deliver(data: Record<string, unknown>, context: ExecutionContext): Promise<CIGatewayResult> {
    return {
      id: this.generateId(),
  ...
  private async retryWithBackoff(command: CIGatewayCommand, context: ExecutionContext): Promise<CIGatewayResult> {
  ...
  private canRetry(command: CIGatewayCommand): boolean {
...
export class CIGatewayValidationError extends Error {
  constructor(public readonly errors: any[]) {
    super(`Validation failed: ${errors.map(e => e.message).join(', ')}`);
    this.name = 'CIGatewayValidationError';
  }
}
