# ValidationExecutor — State Machine Design
...
| `VALIDATING` | Input is being validated by RuleExecutor | → PROCESSING, → ERROR |
| `PROCESSING` | Core logic executing via ValidationReporter and RuleCache | → COMPLETING, → ERROR |
