/**
 * ValidationExecutor — Cell Entity Tests
...
import { ValidationExecutor, ValidationExecutorValidationError } from '../src/validate-exec-cell';
import { ValidationExecutorCommand, ExecutionContext, CellState } from '../src/types';

describe('ValidationExecutor', () => {
  let cell: ValidationExecutor;

  const mockCommand: ValidationExecutorCommand = {
    id: 'cmd-001',
    type: 'test-command',
    payload: { key: 'value' },
  ...
  describe('Online Execution', () => {
    ...
    it('should reject commands without required fields', async () => {
      ...
      await expect(cell.execute(invalidCommand, mockContext)).rejects.toThrow(ValidationExecutorValidationError);
    });
  ...
  describe('Nigeria-First Compliance', () => {
    it('should use Nigeria-optimized timeout defaults', () => {
      const defaultCell = new ValidationExecutor();
