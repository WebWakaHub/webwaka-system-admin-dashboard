...
This document is not policy; it is the blueprint for a machine. A machine designed to make capability reuse the path of least resistance, and to make duplication a mechanical impossibility. Version 1 of this program established the doctrine; Version 2 operationalizes it, binding it to the very mechanics of task creation, approval, and execution.

We will not sacrifice velocity for purity. This program is designed to correct our course without freezing progress. We will implement immediate stabilization rules to stop the bleeding, while progressively consolidating existing duplication through a phased migration strategy. This is not a panic-driven rewrite; it is a calm, deliberate, and industrial-strength implementation of reuse governance that will protect the next decade of WebWaka.
...
5.  **Progressive Consolidation:** Existing duplication is addressed through a phased migration strategy, not a "big bang" rewrite.
...
### Intake System
-   **Submission Template:** `/docs/governance/templates/ARB_SUBMISSION.md`
-   **Required Artifacts:** Completed ARB Submission Form, Reuse Declaration, and any relevant architectural diagrams.
-   **Decision Categories:** Approved, Modify, Rejected.
-   **Ruling Storage Location:** A new `ARB_RULINGS` directory in the `webwaka-governance` repository.
-   **Registry & Task Updates:** The ARB is responsible for ensuring that all rulings are reflected in the Capability Registry and any relevant task definitions.
...
2.  **Reuse Declaration:** The agent completes a Reuse Declaration using the template at `/docs/governance/templates/REUSE_DECLARATION.md`.
3.  **Registry Searched:** The agent searches the Capability Registry for an existing capability that meets the need.
4.  **Found?**
...
6.  **New Capability Proposal:** The agent creates a proposal to create a new capability, which automatically triggers an ARB review.

**Enforcement:** The Chief of Staff is responsible for verifying that a completed and approved Reuse Declaration is attached to every new task before it is activated. Any task without a valid Reuse Declaration is invalid and will be rejected.
...
**Ownership:** Each migration wave will have a designated **Wave Owner** (appointed by the ARB) who is responsible for scope control, verification, consumer mapping, and cutover authority.
...
**Manual to Automated Bridge:** Until Level 4 is reached, enforcement will be manual. The Chief of Staff will perform a weekly `REGISTRY_AUDIT` using the template at `/docs/governance/templates/REGISTRY_AUDIT.md`. The results of this audit will be published to the Master Control Board.
...
## 12. Violations & Consequences

If an agent bypasses the reuse doctrine:

-   **What Happens:** The Chief of Staff will issue a `REUSE_VIOLATION_NOTICE` using the template at `/docs/governance/templates/REUSE_VIOLATION_NOTICE.md`.
-   **Who Intervenes:** The Chief of Staff will intervene and halt the task.
-   **What is Reversed:** The offending task will be deactivated and returned for review.
-   **How it is Recorded:** The violation will be recorded in the Master Control Board and the agent's performance record.

---

## 13. Founder Visibility Layer

The Chief of Staff will provide a weekly report to the Founder on the following metrics:

-   **Reuse Ratio:** Percentage of tasks that reused or extended existing capabilities.
-   **Extension vs. Creation:** Number of extensions vs. number of new capabilities created.
-   **ARB Load:** Number of ARB reviews conducted.
-   **Migration Progress:** Progress of the migration waves.
-   **Repeat Violation Patterns:** Any patterns of repeat violations by agents or teams.

---

## 14. Immediate Prompt Transformation

All task prompts will be updated to include the following mandatory section:

```
## Reuse Declaration

**Instructions:** Before proceeding with this task, you must complete a Reuse Declaration using the template at `/docs/governance/templates/REUSE_DECLARATION.md`. Attach the completed declaration to this task. This task cannot be activated until the declaration is complete and approved.
```

This transforms prompts from "build" to "reuse / extend / justify".
